...
public abstract class BaseDataTableSource extends FlinkTableSource
    ...
    protected FileStoreLookupFunction getFileStoreLookupFunction(
            LookupContext lookupContext, Table table, int[] projection, int[] joinKey) {
        ...
        List<String> bucketKeyFieldNames = ((FileStoreTable) table).schema().bucketKeys();
        boolean useCustomShuffle =
                supportBucketShufflePartitioner(joinKeyFieldNames, bucketKeyFieldNames)
                        && lookupContext.preferCustomShuffle();
        int numBuckets;
        ShuffleStrategy strategy = null;
        if (useCustomShuffle) {
            numBuckets = ((FileStoreTable) table).store().options().bucket();
